...
| **SAL journals** (SAcLJ, SAL Annual Review, SAL Practitioner — hosted at `journalsonline.academypublishing.org.sg`) | Academic/practitioner commentary, including annual case-law review chapters by subject (e.g., Tort, Employment) | **Full-text PDFs retrievable by direct `curl` with a browser User-Agent** despite the site appearing paywalled (confirmed 16 Jul 2026) — see playbook for the URL pattern and the ArticleId-probing enumeration technique. Verified fact: the SAL Ann Rev "Employment Law" chapter is **discontinued** (2020–2022 confirmed present, none found 2023/24 by chapter enumeration); current employment commentary now runs in SAcLJ e-First and SAL Practitioner instead. Do not rely on SMU InK as a mirror — see below. |
| **SMU InK** (`ink.library.smu.edu.sg`) | Open-access mirrors of some SAL journal articles and SMU faculty working papers | **Hard-blocked** (confirmed 16 Jul 2026): an Incapsula/Cloudflare-style JS challenge defeats `curl`, `WebFetch`, and the `r.jina.ai` reader proxy alike; no Wayback snapshots exist. Don't burn agent time here — go to `journalsonline.academypublishing.org.sg` directly instead. |
...
| **Law-firm update pages / Lexology / Mondaq** | Client alerts on new legislation, cases, or enforcement trends | SECONDARY tier only; capture firm name + publication date; expect a Cloudflare JS-challenge block on lawgazette.com.sg (not a plain 403 — see playbook). Confirmed **low-yield specifically for secondment/EAA topics** (16 Jul 2026): a full fetch+grep of Drew & Napier's Legal 500 SG Employment 2024 guide found zero secondment/agency-work/labour-supply hits. |
...
| **Google Books previews** | Snippets of treatises (e.g., Ravi Chandran, *Employment Law in Singapore*, still at 6th ed. 2019 as of 16 Jul 2026) | Useful for a lead/paraphrase, not a quotable pinpoint — treat preview snippets as UNVERIFIED/SECONDARY. **Edition trap** (confirmed 16 Jul 2026): a "7th edition (Dec 2024)" hit for a Chandran book with a similar title is a *different* book — *Introduction to Business Law in Singapore*, not *Employment Law in Singapore* — see the traps list in `verification-protocol.md`. |
...
- A specific paragraph/page from a paywalled treatise (e.g., Ravi Chandran) with no open
  mirror. (SAcLJ/SAL Ann Rev articles are **no longer** in this category as of 16 Jul 2026 —
  full-text PDFs are directly retrievable via `curl` with a browser User-Agent; see the
  playbook.)
