#!/usr/bin/env python3
"""
htm_coins.py — pull the LIVE Sqkii "Hunt The Mouse" silver-coin map circles
and print them ranked by tractability.

Uses only public game data (the same circle list the official map shows every
visitor, via an anonymous guest token). Read-only: no login, no purchases, no
mutations. The paid hint text is NOT fetched.

Usage:
    python3 htm_coins.py              # ranked table
    python3 htm_coins.py --json       # raw machine-readable JSON
    python3 htm_coins.py --all        # include 'verifying' (being-won) coins

Runs anywhere with Python 3 (stdlib only). If you are behind a proxy with a
custom CA (e.g. a sandbox), set SQKII_CA_BUNDLE=/path/to/ca.crt.
"""
